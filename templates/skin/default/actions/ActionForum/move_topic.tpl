...
<h4 class="page-subheader">{$aLang.forum_topic_move} <a href="{$oTopic->getUrlFull()}">{$oTopic->getTitle()}</a></h4>

<form action="" method="POST" enctype="multipart/form-data">
	<input type="hidden" name="security_ls_key" value="{$LIVESTREET_SECURITY_KEY}" /> 

	<p>
		<label for="topic_move_id">{$aLang.forum_topic_move_for}</label>
	...
	<button name="submit_topic_move" class="button button-primary">{$aLang.forum_topic_move}</button>
</form>
