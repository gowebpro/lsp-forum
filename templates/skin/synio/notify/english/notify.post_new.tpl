{if $oUser}The user <a href="{$oUser->getUserWebPath()}">{$oUser->getLogin()|escape:'html'}</a>{else}Guest{/if} left a new reply to topic <b>«{$oTopic->getTitle()|escape:'html'}»</b>, you can read it by clicking on <a href="{$oPost->getUrlFull()}">this link</a><br>
